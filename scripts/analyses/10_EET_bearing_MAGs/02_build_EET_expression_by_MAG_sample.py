...
def find_eet_mag_fasta_files(membership):
    ...
    fasta_table.to_csv(
        OUT_DIR / "EET_MAG_FASTA_files.tsv",
        sep="	",
        index=False,
    )
...
def map_proteins_to_mags(eet_wide, membership, contig_to_mags):
    ...
    mapping.to_csv(
        OUT_DIR / "EET_Protein_ID_to_MAG.tsv",
        sep="\t",
        index=False,
    )
    unresolved.to_csv(
    ...
    included_mags.to_csv(
        OUT_DIR / "EET_MAGs_in_expression_subset.tsv",
        sep="\t",
        index=False,
    )
    ...
    excluded_mags.to_csv(
        OUT_DIR / "EET_MAGs_not_in_expression_subset.tsv",
        sep="\t",
        index=False,
    )
...
def map_sample_columns(sample_columns, reference_samples):
    ...
    mapping.to_csv(
        OUT_DIR / "EET_sample_name_mapping.tsv",
        sep="\t",
        index=False,
    )
...
def reshape_eet_expression(eet_wide, sample_columns, protein_mapping, sample_mapping):
    ...
    long.to_csv(
        OUT_DIR / "EET_gene_expression_by_MAG_sample.tsv",
        sep="\t",
        index=False,
    )
...
def summarise_eet_expression(gene_expression):
    ...
    component.to_csv(
        OUT_DIR / "EET_component_expression_by_MAG_sample.tsv",
        sep="\t",
        index=False,
    )
...
def main():
    ...
    EET_MEMBERSHIP_FILE = OUT_DIR / "EET_MAG_membership.tsv"
    ALTERNATIVE_MODULE_FILE = (
        OUT_DIR / "selected_modules_expression_by_MAG_sample.tsv"
    )
